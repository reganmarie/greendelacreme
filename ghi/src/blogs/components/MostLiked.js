...
export default function MostLiked() {
  ...
  return (
    <div className="flex">
      <div className={`flex flex-col ${window.scrollY <= 80.5 ? "mt-4" : "fixed top-4"} p-4 bg-white rounded-xl drop-shadow z-30 sidebar max-w-[200px]`}>
        <div className="h-full w-full">
          <div className="text-center font-semibold mb-3">Most Liked</div>
          ...
          <div className="mb-3 flex items-top justify-between px-1 md:items-start">
            <div className="mb-2 w-full">
              <div className="flex items-center">
                <a href={`#${blog?.id}`}>
                  <p className="text font-extrabold text-pink-700 hover:underline hover:underline-offset-2 indie break-words">{blog?.title}</p>
                </a>
              </div>
              ...
              <p className="mt-1 text-sm font-medium text-gray-600 md:mt-2 break-words overflow-y-auto">{blog?.body}</p>
            </div>
          </div>
